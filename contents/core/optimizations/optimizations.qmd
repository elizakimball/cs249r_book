...
_How do theoretical neural network models transform into practical solutions, and what techniques bridge implementation gaps?_
...
Given the variety of approaches, different structures within a neural network are pruned based on specific criteria. The primary structures for pruning include neurons, channels, and sometimes entire layers[^entire-lay], each with its unique implications and methodologies. The goal in each approach is to ensure that the reduced model retains as much of the original model's predictive prowess as possible while improving computational efficiency and reducing size.

[^entire-lay]: Earlier layers contain more fundamental features such as edge detection for images which are harder to prune without significant accuracy drops. In contrast, later layers in a neural network tend to be more redundant and often lead to less accuracy loss than earlier layers when pruned.
...
* **Activation-Based Pruning**: This method tracks how often a neuron or filter is activated by storing this information in a parameter called the activation counter. Each time the structure is activated, the counter is incremented. A low activation count suggests that the structure is less relevant.[^low-count]
...
[^low-count]: These neurons with low activation count can be crucial for edge cases and minority classes. In a medical context, neurons that get activated for rare conditions could have low activation counts but are vital for patient safety. It is essential to consider different metrics beyond accuracy when determining the impact of pruning to ensure underrepresented groups are not disproportionately affected.
...
Similar in approximation theme, low-rank matrix factorization (LRMF) is a mathematical technique used in linear algebra and data analysis to approximate a given matrix by decomposing it into two or more lower-dimensional matrices. The fundamental idea is to express a high-dimensional matrix as a product of lower-rank matrices, which can help reduce the complexity of data while preserving its essential structure. Mathematically, given a matrix $A \in \mathbb{R}^{m \times n}$, LRMF seeks matrices $U \in \mathbb{R}^{m \times k}$ and $V \in \mathbb{R}^{k \times n}$ such that $A \approx UV$, where $k$[^k] is the rank and is typically much smaller than $m$ and $n$.

[^k]: The parameter k captures how many hidden patterns or features will be extracted from the data.
...
Moreover, the conundrum of choosing the optimal rank $k$, for the factorization introduces another layer of complexity. The selection of $k$ inherently involves a trade-off[^k-trade] between approximation accuracy and model simplicity, and identifying a rank that adeptly balances these conflicting objectives often demands a combination of domain expertise, empirical validation, and sometimes, heuristic approaches. The challenge is further amplified when the data encompasses noise or when the inherent low-rank structure is not pronounced, making the determination of a suitable $k$ even more elusive.

[^k-trade]: In the context of image data, LRMF will compress a high-resolution image, where the final details are lost and the main features are preserved. A higher k value will represent less compression and more details being retained while a lower k value will result in greater compression and more loss of details.

Handling missing or sparse data, a common occurrence in applications like recommendation systems, poses another substantial challenge.[^chall] Traditional matrix factorization techniques, such as Singular Value Decomposition (SVD), are not directly applicable to matrices with missing entries, necessitating the development and application of specialized algorithms that can factorize incomplete matrices while mitigating the risks of overfitting to the observed entries. This often involves incorporating regularization terms or constraining the factorization in specific ways, which in turn introduces additional hyperparameters that need to be judiciously selected.

[^chall]: Handling missing data raises a similar challenge to completing a puzzle where some pieces are missing. The missing values can be inferred by the patterns in the data or the puzzle.
...
where $Q$ is the quantization operator, $r$ is a real valued input (in our case, an activation or weight), $S$ is a real valued scaling factor, and $Z$ is an integer zero point[^int-zero]. The `Int` function maps a real value to an integer value through a rounding operation. Through this function, we have effectively mapped real values $r$ to some integer values, resulting in quantized levels which are uniformly spaced.

[^int-zero]: **Integer zero point:** a reference value that represents where the value "0" is mapped to in the quantized space. It shifts the range of quantized values so that both positive and negative numbers are represented by unsigned integers.
...
Zero-shot quantization refers to the process of converting a full-precision deep learning model directly into a low-precision, quantized model without the need for any retraining or fine-tuning on the quantized model. The primary advantage of this approach is its efficiency, as it eliminates the often time-consuming and resource-intensive process of retraining a model post-quantization.[^by-lev] By leveraging techniques that anticipate and minimize quantization errors, zero-shot quantization maintains the model’s original accuracy even after reducing its numerical precision. It is particularly useful for Machine Learning as a Service (MLaaS) providers aiming to expedite the deployment of their customer’s workloads without having to access their datasets.

[^by-lev]: While zero-shot quantization is convenient, it comes with several trade-offs. Complex models like transformers will see accuracy loss with this compared to traditional approaches. Additionally, some hardware require specific quantization patterns that zero-shot cannot accommodate, and some model architectures need more careful approaches to work for their unique operations.
...
One hardware platform utilizing quantization is the ARM Cortex-M group of 32-bit RISC ARM processor cores. They leverage fixed-point quantization with power of two scaling factors so that quantization and dequantization can be efficiently done by bit shifting[^defn-bit-shift]. Additionally, Google Edge TPUs, Google's emerging solution for running inference at the edge, is designed for small, low-powered devices and can only support 8-bit arithmetic. Many complex neural network models that could only be deployed on servers due to their high computational needs can now be run on edge devices thanks to recent advancements (e.g. quantization methods) in edge computing field.

[^defn-bit-shift]: **Bit shifting:** a fundamental binary operation that moves the binary digits or bits of a number left or right. Consider 12 which is 1100 in binary. A left and right shift would produce 24 (11000 in binary) and 6 (0110 in binary), respectively. It is much faster form of quantization since it simply moves bits around in memory.
...
The goal here is to find the best architecture in terms of accuracy and hardware efficiency for one fixed target hardware.[^car-ex] For a specific hardware, the Arduino Nicla Vision for example, this category of HW-NAS will look for the architecture that optimizes accuracy, latency, energy consumption, etc.

[^car-ex]: For example, the hardware of a car is designed to have good performance under different driving modes (eco, sport, comfort).
...
Some hardware may have different configurations. For example, FPGAs[^ex-fpga] have Configurable Logic Blocks[^defn-clb] (CLBs) that can be configured by the firmware. This method allows for the HW-NAS to explore different configurations. [@jiang2019accuracy; @yang2020coexploration]

[^ex-fpga]: Information on FPGAs can be found in its [repective section](https://mlsysbook.ai/contents/core/hw_acceleration/hw_acceleration.html#field-programmable-gate-arrays-fpgas).
[^defn-clb]: **CLB:** a physical circuit element on an FPGA chip that can be reconfigured to perform different logical functions. It enables the FPGA to be customized for a specific tasks after manufacturing.
...
Blocking is used to make memory access patterns more efficient. If we have three computations the first and the last need to access cache A and the second needs to access cache B, blocking blocks the first two computations together to reduce the number of memory reads needed.[^groceries]

[^groceries]: Think of blocking as similar to optimizing grocery shopping. By grouping items by sections in the grocery store, walking back and forth is minimized.
...
* **[Clustering](https://www.tensorflow.org/model_optimization/api_docs/python/tfmot/clustering)**: Supports model compression by clustering weights into groups for higher compression rates.[^clust-ex]

[^clust-ex]: Clustering replaces weights of similar values with a representative value for each cluster. Similarly, student grades can be grouped from exact percentage scores to letter grade category, trading exact precision for simpler representation.
