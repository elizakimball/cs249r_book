...
Optimizations like ARM Neon and Intel AVX vector extensions provide power-efficient integer and floating point throughput optimized for "bursty"[^defn-bursty] workloads such as inference [@ignatov2018ai]. While slower than GPUs, CPU inference can be deployed in power-constrained environments. For example, ARM's Cortex-M CPUs now deliver over 1 TOPS of INT8 performance under 1W, enabling keyword spotting and vision applications on edge devices ([ARM](https://community.arm.com/arm-community-blogs/b/architectures-and-processors-blog/posts/armv8_2d00_m-based-processor-software-development-hints-and-tips)).

[^defn-bursty]: **Bursty:** a characteristic of workloads that refer to sporadic and intensive processing rather than continuous computation. Popular examples include speech processing, real-time sensors, or web servers.
...
AI is rapidly evolving, with new model architectures, training methodologies, and software frameworks constantly emerging. For example, Transformers have recently become hugely popular for Natural Language Porcessing [@young2018recent]. Keeping pace with these algorithmic innovations requires hardware-software co-design to adapt platforms and avoid accrued technical debt[^defn-tech-debt] quickly.

[^defn-tech-debt]: **Technical debt:** future complexity and costs associated with choosing an easier but limited solution now rather than a better approach that will have a larger immediate cost. Learn more about [hidden technical debt in ML systems](https://mlsysbook.ai/contents/core/ops/ops.html#hidden-technical-debt-in-ml-systems) and its impact on system design.
...
* **Custom Operations:** Incorporate specialized operations like low-precision INT4 or bfloat16[^defn-bfloat] into models to capitalize on dedicated hardware support.
* **Dataflow Mapping:** Explicitly map model stages to computational units to optimize data movement on hardware.

[^defn-bfloat]: **Brain Floating Point (bfloat16):** a 16-bit floating point number format specifically created by Google Brain for neural network training and inference. It trades precision for a larger dynamic range compared to float16.
...
Attention-based models[^defn-atten-model] have thrived on massively parallel GPUs and ASICs, where their computation maps well spatially, as opposed to RNN architectures[^rnn-arch], which rely on sequential processing. The co-evolution of algorithms and hardware unlocked new capabilities.

[^defn-atten-model]: **Attention-based model:** a type of neural network architecture that emphasizes specific parts of the input data to make decisions. For large language models, the model will calculate an attention score for each word in a sequence to indicate which words are more important for the current task.
[^rnn-arch]: Compared to transformers, RNN architectures are more difficult to parallelize since they process data sequentially.
...
Studies have shown 2-5x higher performance and efficiency gains with algorithm-hardware co-exploration than isolated algorithm or hardware optimization efforts [@suda2016throughput]. Parallelizing the joint development also reduces time-to-deployment[^defn-ttd].

[^defn-ttd]: **Time-to-deployment:** the total time from initial concept to having a working model in production.
...
More extensive prototyping is required to evaluate different hardware-software pairings. The need for rapid, iterative prototypes on FPGAs or emulators[^defn-emulator] increases validation overhead. For example, Microsoft found that more prototypes were needed to co-design an AI accelerator than sequential design [@fowers2018configurable].

[^defn-emulator]: **Emulator:** a software program that mimics the behavior of a computer system (the target system) on another system (the host system).
...
[^defn-ffts]: **Fast Fourier Transform (FFT):** an important algorithm for processing signals and data.
...
The [Open Neural Network Exchange (ONNX)](https://onnx.ai/)[^defn-onnx] is at the forefront of this pursuit, proposing an open and shared ecosystem that promotes model interchangeability. ONNX facilitates the use of AI models across various frameworks, allowing models trained in one environment to be efficiently deployed in another, significantly reducing the need for time-consuming rewrites or adjustments.

[^defn-onnx]: **ONNX:** an open-source format for representing machine learning models. It can be thought of as a translator for AI that helps models speak different framework languages.
...
The rise of hardware-agnostic platforms has also played an important role in democratizing the use of AI. By creating environments where AI applications can be executed on various accelerators, these platforms remove the burden of hardware-specific coding from developers. This abstraction simplifies the development process and opens up new possibilities for innovation and application deployment, free from the constraints of hardware specifications.[^hard-specs]

[^hard-specs]: By relying on platforms generalizable to various accelerators, hardware-agnostic platforms sacrifices the ability to exploit unique hardware capabilities. Performances are reduced compared to hardware-specific optimized code and the memory footprint is usually larger due ot the need to support different hardware types.
...
Emerging hardware technologies are at the cusp of revolutionizing power efficiency in this sector. Photonic computing[^photonic], for instance, uses light rather than electricity to carry information, offering a promise of high-speed processing with a fraction of the power usage. We look deeper into this and other innovative technologies in the "Emerging Hardware Technologies" section, exploring their potential to address current power consumption challenges.

[^photonic]: Photonic computing can save 10-100 times more power than electronic systems. Integrating photonic computing with existing systems is a key challenge.

At the edge of the network, AI accelerators are engineered to process data on devices like smartphones, IoT sensors, and smart wearables. These devices often work under severe power limitations, necessitating a careful balancing act between performance and power usage. A high-performance AI model may provide quick results but at the cost of depleting battery life[^bat-life] swiftly and increasing thermal output, which may affect the device's functionality and durability. The stakes are higher for devices deployed in remote or hard-to-reach areas, where consistent power supply cannot be guaranteed, underscoring the need for low-power-consuming solutions.

[^bat-life]: Devices powered by a battery typically need below 1W to operate.
...
Wafer-scale AI takes an extremely integrated approach, manufacturing an entire silicon wafer[^defn-silwaf] as one gigantic chip. This differs drastically from conventional CPUs and GPUs, which cut each wafer into many smaller individual chips. @fig-wafer-scale shows a comparison between Cerebras Wafer Scale Engine 2, which is the largest chip ever built, and the largest GPU. While some GPUs may contain billions of transistors, they still pale in Comparison to the scale of a wafer-size chip with over a trillion transistors.

[^defn-silwaf]: **Silicon wafer:** a thin, circular piece of silicon that acts as the foundation for integrated circuits and semiconductor devices.


![Wafer-scale vs. GPU. Source: [Cerebras](https://www.cerebras.net/product-chip/).](images/png/aimage1.png){#fig-wafer-scale}

The wafer-scale approach also diverges from more modular system-on-chip designs that still have discrete components communicating by bus[^defn-bus]. Instead, wafer-scale AI enables full customization and tight integration of computation, memory, and interconnects across the entire die.

[^defn-bus]: **Bus:** a communication channel that carries data and control signals between different parts of a system.
...
However, the ultra-large-scale also poses difficulties for manufacturability[^man-ex] and yield with wafer-scale designs. Defects in any region of the wafer can make (certain parts of) the chip unusable. Specialized lithography techniques are required to produce such large dies. So, wafer-scale integration pursues the maximum performance gains from integration but requires overcoming substantial fabrication challenges.

[^man-ex]: Manufacturing a wafer is similar to trying to bake one perfect giant cookie instead of many smaller ones.
...
Chiplets are interconnected using advanced packaging techniques[^pack-tech] like high-density substrate interposers, 2.5D/3D stacking, and wafer-level packaging. This allows combining chiplets fabricated with different process nodes, specialized memories, and various optimized AI engines.

[^pack-tech]: Advanced packaging is crucial for chiplet-based designs. After splitting a larger SoC into multiple chiplets, they must be reconnected at high speed and low latency to restore the communication that would have existed on a single monolithic chip.
...
Neuromorphic computing[^chiplets] is an emerging field aiming to emulate the efficiency and robustness of biological neural systems for machine learning applications. A key difference from classical Von Neumann architectures is the merging of memory and processing in the same circuit [@schuman2022opportunities; @markovic2020physics; @furber2016large], as illustrated in @fig-neuromorphic. The structure of the brain inspires this integrated approach. A key advantage is the potential for orders of magnitude improvement in energy-efficient computation compared to conventional AI hardware. For example, estimates project 100x-1000x gains[^gains] in energy efficiency versus current GPU-based systems for equivalent workloads.

[^chiplets]: While chiplets improve performance at the system-integration level, neuromorphic chips use an entirely new computing paradigm.
[^gains]: By only firing signals when events occur, the neuromorphic systems can avoid constant clock-driven updates to save power.
...
Spiking neural networks (SNNs) [@maass1997networks] are computational models for neuromorphic hardware. Unlike deep neural networks communicating via continuous values, SNNs use discrete spikes[^discrete-spikes] that are more akin to biological neurons. This allows efficient event-based computation rather than constant processing. Additionally, SNNs consider the temporal and spatial characteristics of input data. This better mimics biological neural networks, where the timing of neuronal spikes plays an important role. 

[^discrete-spikes]: These spikes capture the timing aspect of signals, and the SNNs become sensitive to these precise temporal patterns.
...
The field is enabled by advances in organic[^defn-organic] semiconductors and nanomaterials that can be deposited on thin, flexible films. However, fabrication remains challenging compared to mature silicon processes. Flexible circuits currently typically exhibit lower performance than rigid equivalents.[^rigid-eq] Still, they promise to transform electronics into lightweight, bendable materials.

[^defn-organic]: **Organic:** carbon-based. Organic thin-film transistors can be made at lower temperatures than silicon which is essential for producing flexible circuits with plastic substrates that cannot withstand high heat.
[^rigid-eq]: Flexible circuits have lower performance than rigid circuits due to material constraints and structural issues. By repeatedly bending and flexing the materials, microcracks in conductors, delamination between layers, and material fatigue can occur.
...
* 3D stacking can also boost memory density and bandwidth by vertically integrating memory layers with TSV interconnects [@loh20083dstacked].[^tsv] For example, HBM provides 1024-bit wide interfaces.

[^tsv]: Both power and time are saved by stacking memory on top of computer layers since the data does not need to traverse off of the chip.
...
Quantum computers leverage unique phenomena of quantum physics, like superposition[^defn-super] and entanglement[^defn-entang], to represent and process information in ways not possible classically. Instead of binary bits, the fundamental unit is the quantum bit or qubit. Unlike classical bits, which are limited to 0 or 1, qubits can exist simultaneously in a superposition of both states due to quantum effects.

[^defn-super]: **Superposition:** a principle that states a qubit can exist in a combination, or superposition, of both states 0 and 1 at the same time. When the qubit is measured, it no longer has a superposition, and it collapses to either 0 and 1.
[^defn-entang]: **Entanglement:** a quantum phenomenon where measuring a qubit can immediately affect the state of another qubit. This allows qubits to share and process information faster than classical computation.
