...
The transition from experimental to production environments represents a fundamental challenge in scaling machine learning systems. Operational practices reveal essential patterns for maintaining reliability and adaptability throughout the system lifecycle, highlighting critical relationships between deployment architecture and continuous delivery. The implementation of Machine Learning Operations (MLOps) frameworks draws out the key trade-offs between automation, monitoring, and governance that shape system evolution. Understanding these operational dynamics provides insights into managing complex AI workflows, establishing core principles for creating systems that remain robust and responsive in dynamic production environments.
...
MLOps is a systematic approach that combines machine learning, data science, and software engineering to automate the end-to-end ML lifecycle. This includes everything from data preparation and model training to deployment and maintenance. MLOps ensures that ML models are developed, deployed, and maintained efficiently and effectively.
...
[^data-drift]: Data drift can amplify and reinforce historical bias and outdated representations. For example, a facial recognition system trained on one demographic will produce discriminatory outcomes as population demographics shift.
...
Data in its raw form, whether collected from sensors, databases, apps, or other systems, often requires significant preparation before it can be used for training or inference.[^poor-manage] Issues like inconsistent formats, missing values, and evolving labeling conventions can lead to inefficiencies and poor model performance if not systematically addressed. Robust data management practices ensure that data remains high quality, traceable, and readily accessible throughout the ML lifecycle, forming the foundation of scalable machine learning systems.

[^poor-manage]: Poor data management can create compounding technical debt. It is recommended to fix data quality issues during data collection since it can be 10x more expensive if addressed after model deployment.
...
An example workflow has a data scientist using a PyTorch notebook to develop a convolutional neural network (CNN) model for image classification. The [fast.ai](https://www.fast.ai/) library provides high-level APIs to simplify training CNNs on image datasets. The notebook trains the model on sample data, evaluates accuracy metrics, and tunes hyperparameters like learning rate and layers to optimize performance. This reproducible notebook is version-controlled and integrated into a retraining pipeline.
...
After deployment, comprehensive monitoring[^comp-monitor] enables teams to maintain confidence in model and system health. It empowers teams to catch and resolve deviations preemptively through data-driven alerts and dashboards. Active monitoring is essential for maintaining highly available, trustworthy ML systems.

[^comp-monitor]: For best practices, multi-layer monitoring should be implemented at the model layer (accuracy, precision, recall), application layer (latency, throughput), and infrastructure layer (CPU, memory, disk usage).
...
For example, a data scientist may use Weights & Biases[^w-and-b] to analyze an anomaly detection model experiment and share the evaluation results with other team members to discuss improvements. The final model can then be registered with MLflow before handing off for deployment.

[^w-and-b]: Weights & Biases can help compare training runs with different hyperparameters by creating visualizations to inspect which configurations result in the best performance on the validation dataset.
...
Tight coupling between ML model components makes isolating changes difficult. Modifying one part causes unpredictable ripple effects throughout the system. Changing anything changes everything[^change-ex] (also known as CACE) is a phenomenon that applies to any tweak you make to your system. Potential mitigations include decomposing the problem when possible or closely monitoring for changes in behavior to contain their impact.

[^change-ex]: For example, data preprocessing affects feature importance, and feature selection impacts model sensitivity.
...
Understandably, technical debt accumulates naturally in the early stages of model development. When aiming to build MVP[^defn-mvp] models quickly, teams often need more complete information on what components will reach scale or require modification. Some deferred work is expected.

[^defn-mvp]: **Minimum Viable Product model:** a simplified initial version of a model that functions as a starting point for further development. Unlike a baseline model that will only meet the simplest benchmarks, a MPV model includes a proper ML algorithm, a complete pipeline with preprocessing, and minimal feature engineering.
...
In traditional centralized MLOps, data is aggregated into large datasets and data lakes, then processed on cloud or on-prem servers. However, embedded MLOps relies on decentralized[^decentral] data from local on-device sensors. Devices collect smaller batches of incremental data, often noisy and unstructured.[^edge-accuracy] With connectivity constraints, this data cannot always be instantly transmitted to the cloud and needs to be intelligently cached and processed at the edge.

[^decentral]: The decentralized nature of edge devices has several key consequences such as varying data quality standards, inconsistencies caused by unique local phenomena, and need for redundant infrastructure costs.
[^edge-accuracy]: The accuracy of edge devices is limited by environmental conditions, hardware limitations, and connectivity issues.
...
The volume of aggregated data is much lower, often requiring techniques like federated learning across devices to create training sets. The specialized nature of edge data also limits public datasets for pre-training.[^ml-pre-train] With privacy concerns, data samples must be tightly controlled and anonymized where possible.

[^ml-pre-train]: During pre-training, an ML model is initially trained on a large dataset to learn general features before it can be fine-tuned for a specific use case. In the context of edge data, this must be adapted by using smaller, more focused architectures, leveraging transfer learning, and accounting for limited memory and compute.
...
Models must be tested early and often on deployed edge hardware covering diverse configurations. In addition to accuracy, factors like latency, CPU usage, memory footprint, and power consumption are critical evaluation criteria. Models are selected based on tradeoffs between these metrics to meet edge device constraints.[^real-env]

[^real-env]: Establishing realistic test environments for edge conditions requires simulating complex combinations of physical conditions and hardware variations that devices will face in deployment. It is challenging to replicate unpredictable examples of edge environments when devices will have diverse set of deployment settings.
...
Broad device coverage is challenging but critical. Issues specific to certain device types may arise, so monitoring needs to cover the full spectrum. Canary deployments[^defn-canary-dep] help trial monitoring processes before scaling.

[^defn-canary-dep]: **Canary deployments:** a technique where a small subject of devices receive the changes before wider deployment.
...
[^impulse]: Since Edge Impulse is prioritizing ease of use, the high-level abstraction can result in lower efficiency and less optimized models compared to hand-tuned solutions.
